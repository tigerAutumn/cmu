...
@Slf4j
@Component
public class WinWallet extends AbstractEthLikeWallet implements IWallet {
    @Autowired
    LuckyWinCommand command;
    @Value("${newex.luckywin.server}")
    private String serverUrl;

    @Value("${newex.win.withdraw.address}")
    private String winWithdrawAddress;
    @Value("${newex.win.pubkey}")
    private String winPubkey;
    ...
    public void init() {
        ...
        if (StringUtils.isNotBlank(this.winPubkey)) {
            this.node = Bip32Node.decode(this.winPubkey);
        }

        super.setWithdrawAddress(this.winWithdrawAddress);
    }

    @Override
    public CurrencyEnum getCurrency() {
        return CurrencyEnum.WIN;
    }
    ...
    public BigDecimal getDecimal() {
        return CurrencyEnum.WIN.getDecimal();
    }
