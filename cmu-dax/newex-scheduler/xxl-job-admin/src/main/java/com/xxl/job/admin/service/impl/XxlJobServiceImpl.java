...
public class XxlJobServiceImpl implements XxlJobService {
			...
			for (String childJobIdItem: childJobIds) {
				if (StringUtils.isNotBlank(childJobIdItem) && StringUtils.isNumeric(childJobIdItem)) {
					...
					if (childJobInfo.getId() == jobInfo.getId()) {
						return new ReturnT<>(ReturnT.FAIL_CODE, MessageFormat.format(I18nUtil.getString("jobinfo_field_childJobId_limit"), childJobIdItem));
					}
				} else {
					return new ReturnT<>(ReturnT.FAIL_CODE,
							MessageFormat.format((I18nUtil.getString("jobinfo_field_childJobId")+"({0})"+I18nUtil.getString("system_unvalid")), childJobIdItem));
	...
	public ReturnT<Map<String, Object>> chartInfo(Date startDate, Date endDate) {
		...
		// set cache
		// cache 60s
		LocalCacheUtil.set(cacheKey, result, 60*1000);
