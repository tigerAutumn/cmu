...
@Slf4j
@Component
public class EmailBindRateLimitParameterProvider
    ...
    private final static String RATE_LIMIT_KEY_PREFIX = "newex:users:retry-limit:email-bind:";
    ...
    protected String getRateLimitKey(final Type targetType,
                                     final Object body) {
        String key = "";
        if (body instanceof BindEmailVO) {
            key = ((BindEmailVO) body).getEmail();
        }
        log.debug("newex:users:retry-limit:email-bind:{}", key);
