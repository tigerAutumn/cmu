...
@Slf4j
@RestController
@RequestMapping(value = "/v1/boss/asset/currency/perpetual")
public class PerpetualCurrencyController {
    ...
    @Secured({"ROLE_SUPER_ADMIN", "ROLE_ASSET_PERPETUAL_CURRENCY_VIEW"})
    public ResponseResult list(final DataGridPager<AssetCurrencyDTO> pager, @CurrentUser final User loginUser,
        ...
        if (StringUtils.isBlank(biz)) {
            biz = CurrencyBizEnum.PERPETUAL.getName();
        }
    ...
    @OpLog(name = "新增币种")
    @Secured({"ROLE_SUPER_ADMIN", "ROLE_ASSET_PERPETUAL_CURRENCY_ADD"})
    ...
    @OpLog(name = "修改币种")
    @Secured({"ROLE_SUPER_ADMIN", "ROLE_ASSET_PERPETUAL_CURRENCY_EDIT"})
    public ResponseResult edit(@Valid final AssetCurrencyVO assetCurrencyVO, @RequestParam("id") final Integer id) {
        ...
        log.info("edit Perpetual currency info is => {}", assetCurrencyDTO);
    ...
    @Secured({"ROLE_SUPER_ADMIN", "ROLE_ASSET_PERPETUAL_CURRENCY_ONE_STEP"})
