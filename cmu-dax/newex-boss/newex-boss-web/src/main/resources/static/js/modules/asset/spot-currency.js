...
var SpotCurrency = {
    init: function () {
        SpotCurrencyMVC.View.initControl();
        SpotCurrencyMVC.View.bindEvent();
        SpotCurrencyMVC.View.bindValidate();
        SpotCurrencyMVC.View.initData();
    }
};

var SpotCurrencyCommon = {
    baseUrl: BossGlobal.ctxPath + '/v1/boss/asset/currency/spot/',
...
var SpotCurrencyMVC = {
    URLs: {
        add: {
            url: SpotCurrencyCommon.baseUrl + 'add',
            method: 'POST'
        },
        edit: {
            url: SpotCurrencyCommon.baseUrl + 'edit',
            method: 'POST'
        },
        list: {
            url: SpotCurrencyCommon.baseUrl + 'list',
            method: 'GET'
        },
        img: {
            url: SpotCurrencyCommon.baseimgUrl + 'oss-upload',
            method: 'GET'
        },
        OneStep: {
            url: SpotCurrencyCommon.baseUrl + 'one-step',
            method: 'POST'
        },
        getBrokerList: {
            url: SpotCurrencyCommon.brokerBaseUrl + 'list',
            method: 'GET'
        },
        unhold: {
            url: SpotCurrencyCommon.baseUrl + 'unhold',
            method: 'POST'
        }
    },
    Model: {},
    View: {
        initControl: function () {
            ...
            $("#imgfile").filebox({
                onChange: function () {
                    SpotCurrencyMVC.Util.imgupdate();
                },
                prompt: '选择图片',
                buttonText: '&nbsp;选&nbsp;择&nbsp;',
            });

            $("#spotBrokerId").combobox({
                url: SpotCurrencyMVC.URLs.getBrokerList.url,
                method: 'get',
                valueField: "id",
            ...
            $('#currency-biz').combobox({
                ...
                onLoadSuccess: function () {
                    $('#currency-biz').combobox('setValue', 'spot');
                }
            });
            ...
            $('#currency-datagrid').datagrid({
                ...
                rownumbers: true,
                pageSize: 50,
                url: SpotCurrencyMVC.URLs.list.url,
                toolbar: [{
                    text: '新增',
                    iconCls: 'icon-add',
                    handler: function () {
                        SpotCurrencyMVC.Controller.add();
                    }
                }, '-', {
                    text: '编辑',
                    iconCls: 'icon-edit',
                    handler: function () {
                        SpotCurrencyMVC.Controller.edit();
                    }
                }, '-', {
                    text: '冻结(充提、划转)',
                    iconCls: 'icon-lock',
                    handler: function () {
                        SpotCurrencyMVC.Controller.freeze();
                    }
                }, '-', {
                    text: '解冻(充提、划转)',
                    iconCls: 'icon-unlock',
                    handler: function () {
                        SpotCurrencyMVC.Controller.unfreeze();
                    }
                }, '-', {
                    text: '资产解冻',
                    iconCls: 'icon-unlock',
                    handler: function () {
                        SpotCurrencyMVC.Controller.unhold();
                    }
                }, '-', {
                    text: '所有数据',
                    iconCls: 'icon-reload',
                    ...
                    formatter: function (value, row, index) {
                        ...
                        for (var i = 0; i < icons.length; i++) {
                            var tmpl = '<a href="#" title ="${title}" '
                                + 'onclick="SpotCurrencyMVC.Controller.doOption(\'${index}\',\'${name}\')">'
                                + '<img src="${imgSrc}" alt="${title}"/"></a>';
                            var data = {
                                title: icons[i].title,
                                name: icons[i].name,
                                index: index,
                                imgSrc: SpotCurrencyCommon.baseIconUrl + icons[i].name + ".png"
                            };
                ...
                onDblClickRow: function (rowIndex, rowData) {
                    return SpotCurrencyMVC.Controller.edit();
                },
            ...
            $('#currency-dlg').dialog({
                ...
                buttons: [{
                    ...
                    text: '确认',
                    iconCls: 'icon-save',
                    handler: SpotCurrencyMVC.Controller.save
                }]
            });

            $('#currency-unhold-dlg').dialog({
                closed: true,
                modal: false,
                width: 500,
                height: 300,
                ...
                buttons: [{
                    ...
                    iconCls: 'icon-no',
                    handler: function () {
                        $("#currency-unhold-dlg").dialog('close');
                    }
                }, {
                    text: '确认',
                    iconCls: 'icon-save',
                    handler: SpotCurrencyMVC.Controller.saveUnhold
                }]
            });

        },
        bindEvent: function () {
            $('#btn-search').bind('click', SpotCurrencyMVC.Controller.find);
            $('#one-step-frozen').bind('click', SpotCurrencyMVC.Controller.oneStep);
            $('#one-step-unfrozen').bind('click', SpotCurrencyMVC.Controller.oneStep);
        },
    ...
    Controller: {
        ...
        add: function () {
            var options = SpotCurrencyMVC.Util.getOptions();
            ...
            $("#spotBrokerId").combobox('readonly', false);
        },
        edit: function () {
            ...
            if (row) {
                ...
                $("#spotBrokerId").combobox('readonly', true);
        ...
        find: function () {
            var params = SpotCurrencyMVC.Util.getQueryParams();
        ...
        save: function () {
            ...
            if (action === "edit") {
                var row = $('#currency-datagrid').datagrid('getSelected');
                //取得ID传给后台
                options.url = SpotCurrencyMVC.URLs.edit.url + '?id=' + row.id;
            } else {
                options.url = SpotCurrencyMVC.URLs.add.url;
            }
        ...
        freeze: function () {
            SpotCurrencyMVC.Controller.oneStep('冻结', 1);
        },
        unfreeze: function () {
            SpotCurrencyMVC.Controller.oneStep('解冻', 2);
        },
        oneStep: function (tips, type) {
            $.messager.confirm('确认', '确定需要执行一键[' + tips + ']操作?', function (r) {
                if (r) {
                    var url = SpotCurrencyMVC.URLs.OneStep.url;
        ...
        unhold: function () {
            ...
            if (row) {
                var options = SpotCurrencyMVC.Util.getUnholdOptions();
                options.iconCls = 'icon-edit1';
                options.data = row;
                options.title = '资产解冻[' + options.data.symbol + ']';

                EasyUIUtils.openEditDlg(options);

                $('#currencyId').val(options.data.id);
                $('#brokerId').val(options.data.brokerId);
            } else {
                $.messager.alert('警告', '请选中一个币种!', 'info');
            }
        },
        saveUnhold: function () {
            var options = SpotCurrencyMVC.Util.getUnholdOptions();
    ...
    Util: {
        getOptions: function () {
            return {
                ...
                gridUrl: SpotCurrencyMVC.URLs.list.url
            };
        },
        getUnholdOptions: function () {
            return {
                dlgId: '#currency-unhold-dlg',
                formId: '#currency-unhold-form',
                actId: '#modal-action',
                rowId: '#roleId',
                title: '',
                iconCls: 'icon-add',
                ...
                url: SpotCurrencyMVC.URLs.unhold.url,
                gridId: '#currency-datagrid',
                gridUrl: SpotCurrencyMVC.URLs.list.url
            };
        },
        ...
        imgupdate: function () {
            ...
            $.ajax({
                url: SpotCurrencyMVC.URLs.img.url,
                type: 'POST',
                data: formData,
                async: false,
                cache: false,
