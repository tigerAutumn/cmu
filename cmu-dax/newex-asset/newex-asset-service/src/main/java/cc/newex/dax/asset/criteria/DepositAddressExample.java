...
public class DepositAddressExample {
    ...
    public static DepositAddressExample from(final DepositAddress depositAddress) {
        final DepositAddressExample example = new DepositAddressExample();
        if (ObjectUtils.isEmpty(depositAddress)) {
            return example;
        }

        final Criteria criteria = example.createCriteria();

        if (!ObjectUtils.isEmpty(depositAddress.getId())) {
            criteria.andIdEqualTo(depositAddress.getId());
        }
        if (!ObjectUtils.isEmpty(depositAddress.getUserId())) {
            criteria.andUserIdEqualTo(depositAddress.getUserId());
        }
        if (!ObjectUtils.isEmpty(depositAddress.getAddress())) {
            criteria.andAddressEqualTo(depositAddress.getAddress());
        }
        if (!ObjectUtils.isEmpty(depositAddress.getCurrency())) {
            criteria.andCurrencyEqualTo(depositAddress.getCurrency());
        }
        if (!ObjectUtils.isEmpty(depositAddress.getBiz())) {
            criteria.andBizEqualTo(depositAddress.getBiz());
        }
        if (!ObjectUtils.isEmpty(depositAddress.getStatus())) {
            criteria.andStatusEqualTo(depositAddress.getStatus());
        }
        return example;
    }

    public DepositAddressExample() {
